    ...
    while true do
        ...
        if scoreboardOpen then
            for _, player in pairs(GetPlayersFromCoords(GetEntityCoords(PlayerPedId()), 10.0)) do
                ...
                local playerName = QBCore.Functions.GetPlayerData().name
                local playerPed = GetPlayerPed(player)
                local playerCoords = GetEntityCoords(playerPed)
                if Config.ShowIDforALL or playerOptin[playerId].optin then
                    loop = 0
                    if Config.ojxnames then
                    DrawText3D(playerCoords.x, playerCoords.y, playerCoords.z + 1.1, '('..playerId..') ')
                    DrawText3D(playerCoords.x, playerCoords.y, playerCoords.z + 1.0, ''..playerName.. '')
                    else
                        DrawText3D(playerCoords.x, playerCoords.y, playerCoords.z + 1.1, '('..playerId..') ')
                    end
                end
